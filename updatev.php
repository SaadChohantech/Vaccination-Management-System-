            ...
            <div class="content ms-auto">
                <div class="row">
                    <div class="col-sm-4 col-3">
                        <h4 class="page-title" style="color: #19b89d";>Update Vaccine</h4>
                    </div>
...
<form method="POST"  >
  <div class="form-group  ">
    ...
    <input type="text" class="form-control"  aria-describedby="emailHelp" placeholder="Enter Name" name="name" value="<?php echo $value['Name']; ?>">
    <!-- <small id="emailHelp" class="form-text text-muted">We'll never share your email with anyone else.</small> -->
  </div>
  <label >Status</label>
        <select id="dropdown" name="selected_option" class="form-control" value="<?php echo $value['statues']; ?>" >
            <option  >Available</option>
            <option  >Unavailable</option>
  
  <input type="submit" class="btn btn-primary mt-3" value="Update" name="update"  style="background-color:#19b89d; ">
</form>
...
if(isset($_POST['update']))
 {
    $name=$_POST['name'];
    $status=$_POST['selected_option'];
   

    $query="UPDATE `vaccines` SET `Name`='$name',`statues`='$status' WHERE `id`= '$id'";
    $run= mysqli_query($db, $query);
    if ($run==true) {
